...
const LABEL = { bridal: 'Bridal', party: 'Party', festive: 'Festive', designer: 'Designer', blouse: 'Blouse', navratri: 'Navratri' };

/* Collections a piece can belong to. Fetched from /api/collections (one source of
 * truth on the server); this fallback keeps the UI working if that call fails. */
let COLL_OPTS = [
  { key: 'bridal', label: 'Bridal specials' }, { key: 'party', label: 'Party wear' },
  { key: 'festive', label: 'Festive collection' }, { key: 'designer', label: 'Designer wear' },
  { key: 'blouse', label: 'Embroidery blouses' }, { key: 'navratri', label: 'Navratri Special' },
];
const collKeys = s => String(s || '').split(',').map(x => x.trim()).filter(Boolean);
const collShort = k => LABEL[k] || (COLL_OPTS.find(c => c.key === k) || {}).label || k;

async function loadCollections() {
  try {
    const r = await api('/api/collections');
    if (Array.isArray(r.collections) && r.collections.length) COLL_OPTS = r.collections;
  } catch { /* keep fallback */ }
  buildFormCollections();
}
function buildFormCollections() {
  const box = document.getElementById('fCollections');
  if (!box) return;
  box.innerHTML = COLL_OPTS.map(c =>
    `<label class="collchk"><input type="checkbox" value="${c.key}"> ${esc(c.label)}</label>`).join('');
}
function collChips(p) {
  const have = collKeys(p.collection);
  return COLL_OPTS.map(c =>
    `<button type="button" class="collchip" data-collkey="${c.key}" data-id="${p.id}"
       aria-pressed="${have.includes(c.key)}">${esc(collShort(c.key))}</button>`).join('');
}
...
async function loadProducts() {
  ...
  if (!rows.length) {
    $('pRows').innerHTML = `<tr><td colspan="11" class="empty">
  ...
  $('pRows').innerHTML = rows.map(p => `<tr data-row="${p.id}">
    ...
    <td>
      ...
      <span class="muted" style="font-size:.76rem">${esc(p.sku)}</span>
    </td>
    <td class="collcell">${collChips(p)}</td>
  ...
  $('pRows').querySelectorAll('.collchip').forEach(btn => btn.onclick = () => {
    const id = +btn.dataset.id;
    btn.setAttribute('aria-pressed', btn.getAttribute('aria-pressed') === 'true' ? 'false' : 'true');
    const keys = [...document.querySelectorAll(`.collchip[data-id="${id}"][aria-pressed="true"]`)]
      .map(b => b.dataset.collkey);
    markDirty(id, 'collection', keys.join(','));
  });
...
/* ------------------------------ form ------------------------------- */
const F = {
  title: 'fTitle', sku: 'fSku', price: 'fPrice', mrp: 'fMrp',
...
function readForm() {
  ...
  v.collections = [...document.querySelectorAll('#fCollections input:checked')].map(i => i.value);
...
function fillForm(p) {
  ...
  const have = new Set(collKeys(p?.collection));
  document.querySelectorAll('#fCollections input').forEach(i => { i.checked = have.has(i.value); });
  hint();
...
async function refreshAll() {
  try { await Promise.all([loadStats(), loadProducts(), loadOrders(), loadReviews(), loadDiscount(), loadCollections()]); }
