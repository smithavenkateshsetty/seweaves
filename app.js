...
// A piece can be in several collections ("designer,navratri") — join their labels.
const collLabels = s => String(s || '').split(',').map(k => COLLECTION_LABEL[k.trim()] || k.trim())
  .filter(Boolean).join(' \u00b7 ');
...
function tile(p) {
  ...
  return `<article class="tile" data-tile="${p.id}">
    ...
    <div class="meta">
      <p class="sub">${esc(collLabels(p.collection) || COLLECTION_LABEL[p.collection] || p.collection)}</p>
...
window.SeWeaves = { Cart, openBag, rupees, esc, thumb, COLLECTION_LABEL, collLabels, priceHTML };
